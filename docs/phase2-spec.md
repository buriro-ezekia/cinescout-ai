...
The Phase 2 candidate pipeline is:
...
Google ADK `SequentialAgent` is the orchestration mechanism because the five stages have a fixed dependency order. Each specialist must be created through a factory function so separate pipeline instances never reuse child agents that already belong to another parent.
...
The pipeline must use these output keys exactly:
...
Later specialists must consume prior outputs through ADK session state rather than relying on hidden coupling between modules.
...
The existing Phase 1 Parallel configuration remains unchanged. Phase 2 creates its own MCP toolset instance through a factory so the Phase 1 root agent is not modified or re-parented.
...
- the Phase 2 pipeline may be imported and structurally validated;
...
1. A five-stage `SequentialAgent` candidate exists in the exact specialist order defined above.
2. Every specialist has a distinct output key matching the shared state contract.
3. Specialist agents are created through factory functions and repeated pipeline construction produces fresh agent instances.
4. Only the Evidence Verifier has a Parallel MCP toolset.
5. The Evidence Verifier is explicitly instructed to research externally verifiable claims and preserve evidence uncertainty.
6. The Report Synthesiser is explicitly constrained to the existing five-section response contract.
7. The Phase 1 `app/agent.py` entry point remains unchanged by Phase 2 implementation.
8. Local readiness includes a Phase 2 structural contract check that makes zero Gemini calls and zero external search calls.
9. Automated tests verify orchestration order, state keys, tool ownership, factory isolation and the at-no-cost execution boundary.
10. README and Phase 2 documentation describe the architecture accurately in professional UK English.

## Deferred live acceptance

The later credentialled acceptance test must demonstrate one complete run in which Gemini executes the five-stage pipeline, the Evidence Verifier invokes Parallel Search MCP at runtime, evidence reaches the downstream risk and synthesis stages, and the final response preserves sources and uncertainty.
